...
## Personal Reproduction Baseline

This repository also contains my personal reproduction work for the VLMOD MonoMulti3D-ROPE evaluation.

The script:

```text
run_monomulti3d_submission.py
```

implements an offline, rule-based baseline. It reads the released training annotations and matches natural-language descriptions with structured 3D object records using properties such as:
...
The script generated prediction files for 300 test samples. My local positive-label validation result was 96.72% on the released training annotations. This is a local validation result and not an official leaderboard score.

The dataset files and generated submission files are not included in this repository.
```
